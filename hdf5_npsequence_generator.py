...
class SequenceGenerator():
  def __init__(self):
    self.dimension = 10
    self.batch_stream_length = 800  #length of words in one h5 file
    self.batch_num_streams = 8 #number of video sentences
  ...
  def init_streams(self):
    ...
    self.stream_indices = [0] * self.batch_num_streams #initialize the index of video sentences in one video
    self.reset_stream(0)
    self.streams_initialized = True

  def reset_stream(self, stream_index): #####//get the cont_sentence, target_sentence, feature from get_streams, to provide to self.streams
    streams = self.get_streams()    ### get the
    stream_names = sorted(streams.keys()) ####// get the cont_sentence, target_sentence,frame_fc7,etc
    ...
    stream_length = len(streams[stream_names[0]])####length of cont_img
    for k, v in streams.iteritems():
      if isinstance(v, np.ndarray):
        assert stream_length == v.shape[0]
      else:
        assert stream_length == len(v) 
      self.streams[stream_index][k] = v#####set the streams = videoid, stream_index = sentence number, k = feature,cont_sentence,etc,v =the content of k
    self.stream_indices[stream_index] = 0######///stream_index is the cont_sentence,target_sentence,feature,etc
  ...
  def get_next_batch(self, truncate_at_exhaustion=True): #exhaust = processed data
    if not self.streams_initialized:####initilize the stream 0
      self.init_streams()
    batch_size = self.batch_num_streams * self.batch_stream_length
    batch = {}### batch[name] contain the output of get_stream, the batch[name] : left dimension: video num, right dimension: video sentences, contain all the values, such as cont_img, etc
    batch_indicators = np.zeros((self.batch_stream_length, self.batch_num_streams))
    for name in self.substream_names:
      batch[name] = self.get_pad_value(name) * np.ones_like(batch_indicators)###the size of batch is length of get_stream out*video nums * sentence num of per video
    exhausted = [False] * self.batch_num_streams
    all_exhausted = False
    reached_exhaustion = False
    num_completed_streams = 0
    for t in range(self.batch_stream_length):  #### for one h5 file
      all_exhausted = True
      for i in range(self.batch_num_streams): ### batch number
        if not exhausted[i]:
          if self.streams[i] is None or \
              self.stream_indices[i] == len(self.streams[i][self.substream_names[0]]):### when stream indices equal to max_words
            ...
            if not reached_exhaustion or not truncate_at_exhaustion:
              ...
              continue
          for name in self.substream_names:   ## e
          ...
          if self.stream_indices[i] == len(self.streams[i][self.substream_names[0]]):### if has itered the max_word times of iteration
...
class HDF5SequenceWriter():
  def __init__(self, sequence_generator, output_dir=None, verbose=False):
    ...
    self.filenames = []
    self.output_vidid_dir = '%s/list' % output_dir
    if not os.path.exists(self.output_vidid_dir):
      os.makedirs(self.output_vidid_dir)
  ...
  def write_batch(self, stop_at_exhaustion=False):
    ...
    vididname = '%s/id_%d.txt' % (self.output_vidid_dir, batch_index)
    #vidid_file = open(vididname, 'w')
