...
BUFFER_SIZE = 1 # TEXT streams
BATCH_STREAM_LENGTH = 600
SETTING = '/media/llj/storage/processed-data-ms'
OUTPUT_DIR = '{0}/hdf5_inception_concat_nocnn_origin/buffer_{1}_s2vt_{2}'.format(SETTING, BUFFER_SIZE, MAX_WORDS)
VOCAB = '%s/yt_coco_mvad_mpiimd_vocabulary.txt' % SETTING
OUTPUT_DIR_PATTERN = '%s/%%s_batches' % OUTPUT_DIR
FRAMEFEAT_FILE_PATTERN = './concat_fusion_{0}'
SENTS_FILE_PATTERN = './sents_{0}_lc_nopunc.txt'
OUT_FILE_PATTERN = \
'./rawcorpus_inception_nocnn_origin/{0}/s2vt_vgg_{0}_sequence.txt'
OUT_CORPUS_PATH = './rawcorpus_concat_nocnn_origin/{0}'
...
def preprocess_dataset(split_name, data_split_name, batch_stream_length,
  ...
  vid_id_order_outpath = '%s/yt_s2vtinception_%s_vidid_order_%d_%d.txt' % \
  (out_path, data_split_name, BUFFER_SIZE, MAX_WORDS)
  frame_sequence_outpath = '%s/yt_s2vtinception_%s_sequence_%d_%d_recurrent.txt' % \
  (out_path, data_split_name, BUFFER_SIZE, MAX_WORDS)
